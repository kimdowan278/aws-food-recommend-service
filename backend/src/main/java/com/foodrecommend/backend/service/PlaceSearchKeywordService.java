...
public class PlaceSearchKeywordService {
    ...
    public String buildLocationSearchKeyword(FoodCategory category, FoodType foodType) {
        return toLocationFoodKeyword(category, foodType);
    }

    private String toLocationFoodKeyword(FoodCategory category, FoodType foodType) {
        if (foodType != null && foodType != FoodType.ETC) {
            return switch (foodType) {
                case TTEOKBOKKI -> "떡볶이";
                case KIMBAP -> "김밥";
