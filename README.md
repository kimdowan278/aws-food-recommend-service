...
본 프로젝트는 다음과 같은 구조로 실행된다.

- 프론트엔드: HTML / CSS / JavaScript (Live Server 실행)
- 백엔드: Spring Boot (AWS EC2에서 상시 실행 중)

---

## 2. 프로젝트 다운로드

GitHub 저장소에서 프로젝트를 다운로드한다.
또는 ZIP 파일로 다운로드 후 압축을 해제한다.

## 3. 프론트엔드 실행 방법 (Live Server)
VS Code에서 프로젝트 폴더를 연다.
index.html 또는 frontend/index.html 파일을 실행하여 Live Server로 실행한다.

## 4. 백엔드 실행 상태

본 프로젝트의 백엔드 서버는 AWS EC2에 배포되어 있으며 항상 실행 중이다.
따라서 별도의 설치 없이 API 서버를 바로 사용할 수 있다.
...
사용자는 웹 페이지를 통해 다음과 같은 흐름으로 서비스를 이용한다.

음식 이미지 업로드
지역 및 메모 입력 (선택 사항)
"분석하기" 버튼 클릭
음식 카테고리 및 라벨 추출
Kakao Local API를 통한 맛집 추천 수행

## 6. 주의사항
백엔드 서버는 AWS EC2에서 항상 실행 중이어야 한다.
인터넷 연결이 필요하다.
Live Server는 프론트엔드 실행 용도로만 사용된다.

## 7. 요약

본 프로젝트는 GitHub에서 소스 코드를 다운로드한 후,
프론트엔드는 Live Server로 실행하고 백엔드는 AWS EC2에서 실행 중인 서버와 연동하여 사용하는 구조이다.
